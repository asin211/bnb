...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
  ...
  if (empty($id) or !is_numeric($id)) {
    echo "<h5>Invalid Customer ID</h5>"; //simple error feedback
    exit;
  }
}
...
if (isset($_POST['submit']) and !empty($_POST['submit']) and ($_POST['submit'] == 'Delete')) {
  ...
  if ($error == 0 and $id > 0) {
    ...
    echo "<h5>Customer details deleted</h2>";
  } else {
...
<?php
} else echo "<h5>No Customer found, possbily deleted!</h5>"; //suitable feedback
