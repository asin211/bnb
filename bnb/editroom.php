...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
  ...
  if (empty($id) or !is_numeric($id)) {
    echo "<h5>Invalid room ID</h5>"; //simple error feedback
    exit;
  }
}
...
if (isset($_POST['submit']) and !empty($_POST['submit']) and ($_POST['submit'] == 'Update')) {
  ...
  if ($error == 0 and $id > 0) {
    ...
    echo "<h5>Room details updated.</h5>";
...
?>
  ...
  <form method="POST" action="editroom.php">
    ...
    <p>
      ...
      <textarea type="text" id="description" name="description" rows="4" cols="30" minlength="5" maxlength="200" required><?php echo $row['description'] ?></textarea>
    </p>
    <p>
...
<?php
  ...
  echo "<h5>room not found with that ID</h5>"; //simple error feedback
}
